...
const TestPre = require('../data_model/testpre');
const TestPost = require('../data_model/testpost');


// getAnalystPostReport
const getAnalystPostReport = async (req, res) => {
    const user = req.user;
    const data = req.body;
    ...
    if(user)
    {
        if(user.user_role == analyst)
        {
            ...
            const test_post = await TestPost.getTestReport(data);

            res.status(200).json({status:"Success", data:test_post, test:test_data, test_pre:test_pre});
...
module.exports = { getAnalystPostReport }